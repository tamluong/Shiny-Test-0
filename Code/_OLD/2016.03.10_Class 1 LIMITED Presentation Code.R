...
#### C. Lists ####
  ...
  #list of reg1_summary components using the names() function
  names(reg1_summary)
...
#### D. Data Frames ####
  ...
  input <- "//AGBOSNAS1/DATA/SHARE/R/2016/Class 1/Code & Examples/Input"
  setwd(input)
  mytable <- read.table("baseballFull.csv", header=TRUE,sep=",",quote="\"")
  ...
  temp <- "//agbosnas1/data/Share/R/2016/Class 1/Code & Examples/Temp"
  setwd(temp)
  save( baseball, file = "BaseballDataset.RData" )
  
  #read in BaseballDataset
  load("BaseballDataset.RData")
  ...
  baseballBOS2 <- subset( baseball, baseball$team == "BOS", 
                          c(name, team, position, hits) )
