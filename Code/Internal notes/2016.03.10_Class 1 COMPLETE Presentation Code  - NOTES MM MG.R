...
####Introduction#####
  ...
  #MM: Potentially add str here?
  
  setwd(input)
  #assuming the "Code & Examples" folder has the 
  #following subfolders: Input, Output, and Temp
  #MM: This is imported as data frame. Either voice over or change order so it's synchronized with presentation.
  ...
  #Example graphics
  #MM: Have them pre-installed before class?
  ...
  #demo(rgl)
  #demo(abundance)
  #MM: Have not worked much with these graphics. Simpler ones to show? Potentially more standard?
  ...
  #Help Example
  ?lm
  #MM: Show how it offers how function works if you type it in
  #Tab example
  #MM: Does this work?
  l
...
#### A. Scalars & Vectors ####
#MM: Get to examples of all the types (make sure go over logical vector)
#MM: Once we begin talking about vectors, maybe introduce types. Introduce typeof
  #Vector Creation#
  pi
  is.vector(pi)
  ...
  #MM: Is this necessary to teach? -- move down to data frames
  assign("x",c(1,2,3,4))
  x
  
  #using the combine function to create a vector from other vectors
  #MM: This is not the combine function?
  x1 <- c(3,2,1)
  x2 <- c(4,5,6)
  x3 <- c(7,8,9)
  x4 <- c(x1,x2,x3)
  ...
  x <- seq(1,20,2)
  x
  #MM: Could add ?seq here to show how the help window can be used in this simple example.
  ...
  H <- c(5,6,7,8)
  h
  #MM: This point has already been made clear in above code.
  H
...
#### Vector Functions ####  
  ...
  #Create a vector of 100 random normally distributed values with mean 0 and standard deviation 1
      #MM Add ?rnorm and interact with class
  ...
  sd(x)
  #MM: Have not used this function. Does it not guarantee mean and SD will be exactly 0 and 1?
...
#### C. Lists ####
  #MM: add in unlist at some point -- typeof too!
  #creating a list
  x <- c("one","two","three")
  y
  ...
  #MM: Use ?lm here to go over regression
  ...
  reg1_coef
  #MM: Could reference residuals here 
...
#### D. Data Frames ####
  ...
  #Method A: Similarly to a vector/matrix
    ...
    #MM: Nix this line -- DO NOT NIX  -- RENIX
    baseball[-c(2),]
  ...
  #Method B: Using dataframe properties
    ...
    #MM: Add a -"team" example -- PURPOSE OF RENIX ABOVEZ
    baseball[,"team"]
  ...
  #Creating variables in a Dataframe
    #MM: Actually calculate real stats
  ...
  #Subsetting a Dataframe
    ...
    #MM: %in% here? Ask Will if jumping ahead too much
